...
- `write_file FILE CONTENT` writes a file that is entirely ours (a launcher script, `~/.p10k.zsh`), and `file_matches FILE CONTENT` is its check for `module_is_applied`. `install_desktop_entry FILE CONTENT` is `write_file` for a launcher entry: a no-op when current, and it refreshes the MIME cache.
...
- `download_verified URL FILE SHA256` downloads over HTTPS only and fails unless the file matches the pinned checksum; a file that already matches is kept. `checksum_ok FILE SHA256` is the check alone.
