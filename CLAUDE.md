...
- `pkg_install` is for official repo packages and never refreshes the database (`-Sy` alone would be a partial upgrade); on a stale database run `omarchy-update` first. `aur_install` runs `yay` as the regular user (never wrap it in `as_root`) and verifies with `pacman -Q` afterwards, because `yay` exits 0 even when its final pacman step fails.
